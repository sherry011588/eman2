	...
	for itr in range(options.niter):
		...
		for projs,pjr,pji,xf in trainset:   #grd,
			...
			with tf.GradientTape() as gt:
				...
				z_mean = tf.keras.layers.Dense(8, name="z_mean")(conf)#options.nmid
				z_log_var = tf.keras.layers.Dense(8, name="z_log_var")(conf)#options.nmid
				...
				loss = tf.reduce_mean(tf.keras.losses.binary_crossentropy(projs, pout))####,axis=1, axis=(1, 2)
